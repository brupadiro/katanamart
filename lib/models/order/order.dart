...
class Order {
  ...
  Order._fromPrestashop(Map<String, dynamic> parsedJson) {
    try {
      ...
      int? currentState =
          int.tryParse(parsedJson['current_state']?.toString() ?? '');

      if (currentState != null) {
        switch (currentState) {
          case 2: // Payment accepted
          case 3: // Processing in progress
          case 9: // Remote payment accepted
          case 10: // Awaiting for PayPal payment
          case 12: // Awaiting Cash On Delivery validation
          case 13: // Awaiting check payment
          case 14: // Awaiting bank wire payment
          ...
          case 8: // Payment error
            ...
            break;
          case 15: // On backorder (paid)
          case 16: // On backorder (not paid)
      ...
      createdAt =
          DateTime.parse(parsedJson['date_add'] ?? DateTime.now().toString());
      total = double.tryParse(parsedJson['total_paid'].toString());
      paymentMethodTitle = parsedJson['payment'];
      shippingMethodTitle = parsedJson['id_carrier']?.toString() ?? '';
